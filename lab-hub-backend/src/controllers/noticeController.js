  ...
  createNotice: async (req, res) => {
    ...
    try {
      ...
      if (!title || !content || !category) {
        return response.error(res, '공지사항 제목, 본문 및 카테고리는 필수 입력 항목입니다.', 400);
      }
      ...
      if (!allowedCategories.includes(category)) {
        return response.error(res, '올바르지 않은 공지사항 카테고리 설정입니다.', 400);
      }
      ...
      if (attachments && attachments.length > 0) {
        for (const file of attachments) {
          if (!file.filename || !file.storageType) {
            return response.error(res, '첨부파일의 파일명 및 스토리지 타입(drive/nas)은 필수입니다.', 400);
          }
          
          if (!['drive', 'nas'].includes(file.storageType)) {
            return response.error(res, '스토리지 타입은 drive 또는 nas만 허용됩니다.', 400);
          }

          // 구글 드라이브 형태인데 링크가 누락되었거나, NAS 형태인데 절대경로가 누락된 경우를 하드 블록합니다.
          if (file.storageType === 'drive' && !file.fileUrl) {
            return response.error(res, '클라우드 드라이브(drive) 저장 방식은 fileUrl이 필수입니다.', 400);
          }
          if (file.storageType === 'nas' && !file.filepath) {
            return response.error(res, '자체 스토리지(nas) 저장 방식은 filepath가 필수입니다.', 400);
          }
        }
      }
      ...
      return response.success(res, '공지사항 및 첨부파일 메타데이터가 무결하게 등록되었습니다.', {
        notice,
      ...
      return response.error(res, '서버 오류로 공지사항 등록에 실패했습니다.', 500);
    } finally {
  ...
  getNoticeDetail: async (req, res) => {
    try {
      ...
      if (!rows || rows.length === 0) {
        return response.error(res, '존재하지 않거나 삭제된 공지사항 게시글입니다.', 404);
      }
      ...
      return response.success(res, '공지사항 상세 데이터 및 첨부파일 조회가 완료되었습니다.', {
        notice: noticeDetail
      }, 200);

    } catch (error) {
      console.error('공지사항 상세 조회 중 예외 에러 발생:', error);
      return response.error(res, '서버 오류로 공지사항을 조회하지 못했습니다.', 500);
    }
  }
};
